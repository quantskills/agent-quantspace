...
def vr2_sortino_downside_momentum(
    ...
    值，与 volatility_inv（1/全样本 std）和 downside_semivariance_premium
    （1/下行半方差，无趋势项）不同：本因子显式融合「趋势方向」与「下行
    风险调整」。
...
def vr2_vol_term_structure_tanh(
    ...
    险正在积聚。第一轮的 volatility_regime 直接用比值（线性），跨截面区分
    力弱。本因子用 tanh(short/long - 1) 做非线性变换，使「适度贴水」与
...
def vr2_atr_close_zscore(
    ...
    位时风险已释放或过度。第一轮未涉及 ATR。本因子用 ATR 的滚动 z-score
    取负，使「低 ATR 压缩」资产排序靠前（低风险溢价）。与 volatility_inv
    （1/收益 std，基于收盘价）不同，ATR 包含 high/low 信息，对日内波幅更
    敏感，是「路径波动率」而非「收盘波动率」。
