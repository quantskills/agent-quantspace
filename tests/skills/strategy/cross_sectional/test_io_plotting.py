...
from skills.strategy.cross_sectional.io import load_price_data
from skills.strategy.cross_sectional.plotting import plot_backtest_results, plot_weight_heatmap
...
def test_plotting_helpers_return_figures_and_validate_inputs(tmp_path) -> None:
    ...
    fig = plot_backtest_results(
        result_df, {"total_return": 0.05}, show_plot=False, save_path=str(figure_path)
    )
    heatmap = plot_weight_heatmap(
        pd.DataFrame({"AAA": [1.0, 0.0, 1.0]}, index=index), show_plot=False
    )
