...
def test_compare_strategies_uses_explicit_data_and_collects_metrics(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "skills.strategy.cross_sectional.modular_backtester.ModularBacktester",
        FakeBacktester,
    )
    ...
    assert seen_kwargs[0]["data"] is panel
    assert seen_kwargs[0]["slippage_bp"] == 2.0


def test_compare_strategies_requires_explicit_data() -> None:
    with pytest.raises(ValueError, match="explicit data"):
        compare_strategies(
            None,  # type: ignore[arg-type]
            [{"name": "missing", "factor_configs": []}],
            slippage_bp=2.0,
        )
